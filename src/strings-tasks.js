...
function removeFirstOccurrences(str, value) {
  ...
  const str1 = str.slice(0, startIndex);
  const str2 = str.slice(startIndex + value.length);
...
function isPalindrome(str) {
  const newStr = str.replaceAll(/\W+/g, '').toUpperCase();
...
function encodeToRot13(str) {
  return str
    ...
    .map((char) => {
      ...
      if (code <= 77 || (code >= 97 && code <= 109)) {
        return String.fromCharCode(code + 13);
      }
      if ((code > 77 && code <= 90) || code > 109) {
        return String.fromCharCode(code - 13);
      }
      return String.fromCharCode(code);
    })
